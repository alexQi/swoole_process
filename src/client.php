...
class Client {
    ...
    public function connect() {
        ...
        $params = [$this->_config['host'], $this->_config['port'], self::$errNum, self::$errStr];
        ...
        if (!empty(self::$errNum) || !empty(self::$errStr)) {
            $this->_error("{".self::$errNum.".}: {".self::$errStr."}");
        }
     ...
     *
     * @param integer $id The id of the job.
     * @param string|integer $pri *New* priority to assign to the job.
     * @return boolean `false` on error, `true` on success.
     */
